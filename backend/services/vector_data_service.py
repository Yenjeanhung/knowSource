...
from providers.vector_store import enrich_vector_index_records, get_vector_store_provider_name
...
class VectorDataService:
    ...
    async def list_records(
        ...
        if query:
            like = f"%{query.strip()}%"
            filters.append(or_(File.name.like(like), Chunk.content.like(like), Chunk.embedding_id.like(like)))
        ...
        stmt = (
            ...
            .join(File, Chunk.file_id == File.id)
            .join(KnowledgeBase, File.kb_id == KnowledgeBase.id)
            .order_by(File.created_at.desc(), Chunk.chunk_index.asc())
    ...
    async def similarity_test(kb_id: str, query: str, top_k: int = 10) -> dict:
        ...
        docs_with_scores = vectorstore.similarity_search_with_score(query, k=max(1, min(top_k, 20)))
    ...
    @staticmethod
    async def export_summary(db: AsyncSession, kb_id: str | None = None, fmt: str = "json"):
        records = await VectorDataService.list_records(db, kb_id=kb_id, limit=200, offset=0)
        items = records["items"]

        kb_groups = defaultdict(lambda: {"kb_name": "", "files": defaultdict(int), "synced": 0, "unsynced": 0})
        for item in items:
            ...
            "knowledge_bases": [
                {
                    ...
                    "files": [{"file_name": file_name, "chunk_count": chunk_count} for file_name, chunk_count in data["files"].items()],
                }
        ...
        if fmt == "md":
            ...
            for kb in summary["knowledge_bases"]:
                ...
                for file_info in kb["files"]:
                    lines.append(f"- {file_info['file_name']}: {file_info['chunk_count']} chunks")
                lines.append("")
            return "\n".join(lines)
